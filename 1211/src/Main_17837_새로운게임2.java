...
public class Main_17837_새로운게임2 {
	...
	// 1과 바뀐 부분
	private static void go(int r, int c, int nr, int nc, Piece now) {
		
		int first = now.getIdx();
		...
		while(prior.size()>first) {
			Piece tmpP = prior.get(first);
			tmpP.setR(nr);
			tmpP.setC(nc);
			tmpP.setIdx(idx++);
			next.add(tmpP);
